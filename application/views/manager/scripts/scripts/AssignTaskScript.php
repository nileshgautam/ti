	...
	$(function(){
		...
		$("[name=project]").change(function(){
			...
			if(projectId!="")
			{
				...
				request.open("POST","<?php echo base_url(); ?>Manager/getTaskByprojectAndManager");
				request.send(formData);
			}
		});

		$("#save").click(function(){
			var project = $("[name=projectId]");
			var task = $("[name=taskId]");
			var startDate = $("[name=startDate]");
			var endDate = $("[name=endDate]");
			var hours = $("[name=hours]");
			var userId = $("[name=users]");
			var error = "";
			...
			request.open("POST","<?php echo base_url(); ?>Manager/allocateTaskToUser");
			request.send(formData);
